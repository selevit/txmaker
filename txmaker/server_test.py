...
async def test_create_if_no_available_utxos(client: TestClient, fake_server_client_factory: Any,
                                            monkeypatch: MonkeyPatch) -> None:
    def blockchain_info_mock_handler(request: web.Request) -> web.Response:
        return web.Response(body='No free outputs to spend', status=500)

    fake_server_client = await fake_server_client_factory(
        hosts=['testnet.blockchain.info', 'blockchain.info'],
        routes=[
            web.get('/unspent', blockchain_info_mock_handler),
        ]
    )
...
async def test_create_if_no_available_confirmed_utxos(client: TestClient, fake_server_client_factory: Any,
                                                      monkeypatch: MonkeyPatch) -> None:
    def blockchain_info_mock_handler(request: web.Request) -> web.Response:
...
async def test_create_transaction_if_insufficient_funds(client: TestClient, fake_server_client_factory: Any,
                                                        monkeypatch: MonkeyPatch) -> None:
    def blockchain_info_mock_handler(request: web.Request) -> web.Response:
        return web.json_response({
            "unspent_outputs": [
...
async def test_create_transaction_with_p2sh_input(client: TestClient, fake_server_client_factory: Any,
                                                  monkeypatch: MonkeyPatch) -> None:
    response = await client.post('/payment_transactions', json={
...
async def test_create_transaction_if_both_p2sh_and_p2pkh_outputs(client: TestClient, fake_server_client_factory: Any,
                                                                 monkeypatch: MonkeyPatch) -> None:
    def blockchain_info_mock_handler(request: web.Request) -> web.Response:
        assert request.query['active'] == 'mfetsqmVYU1m3w1MNXaDg4cvGYwXJB47Kx'
...
async def test_create_transaction_if_no_change(client: TestClient, fake_server_client_factory: Any,
                                               monkeypatch: MonkeyPatch) -> None:
    def blockchain_info_mock_handler(request: web.Request) -> web.Response:
        assert request.query['active'] == 'mqdofsXHpePPGBFXuwwypAqCcXi48Xhb2f'
...
async def test_create_transaction_with_many_inputs(client: TestClient, fake_server_client_factory: Any,
                                                   monkeypatch: MonkeyPatch) -> None:
...
async def test_create_transaction_with_p2pkh_output(client: TestClient, fake_server_client_factory: Any,
                                                    monkeypatch: MonkeyPatch) -> None:
